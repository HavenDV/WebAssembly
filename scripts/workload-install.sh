...
while [ $# -ne 0 ]; do
    ...
    case "$name" in
        ...
        -h|--help)
            ...
            echo "WebAssembly Workload Installer"
...
function install_webassemblyworkload() {
    ...
    echo "Done installing WebAssembly workload $MANIFEST_VERSION"
    echo ""
}
...
if [ -z "$INSTALLED_DOTNET_SDKS" ]; then
    echo ".NET SDK version 6 or later is required to install WebAssembly Workload."
else
