...
namespace WebAssembly.Build.Tasks
{
    public class ResourceXmlWriter : Task
    {
        ...
        public override bool Execute()
        {
            ...
            XmlNode rootNode = doc.CreateElement(STR_RES, "http://webassembly.org/ns/rm");
            doc.AppendChild(rootNode);

            XmlElement groupImageNode = doc.CreateElement("group-image", "http://webassembly.org/ns/rm");
            groupImageNode.SetAttribute(STR_FOLDER, STR_CONTENTS);
            rootNode.AppendChild(groupImageNode);

            XmlElement groupLayoutNode = doc.CreateElement("group-layout", "http://webassembly.org/ns/rm");
            groupLayoutNode.SetAttribute(STR_FOLDER, STR_CONTENTS);
            rootNode.AppendChild(groupLayoutNode);

            XmlElement groupSoundNode = doc.CreateElement("group-sound", "http://webassembly.org/ns/rm");
            groupSoundNode.SetAttribute(STR_FOLDER, STR_CONTENTS);
            rootNode.AppendChild(groupSoundNode);

            XmlElement groupBinNode = doc.CreateElement("group-bin", "http://webassembly.org/ns/rm");
            ...
            foreach (var dir in contentsDir.GetDirectories())
            {
                ...
                if (!nameParts.Item1.Equals("default_All") || !nameParts.Item2.Equals("All"))
                {
                    ...
                    {
                        XmlElement node = doc.CreateElement("node", "http://webassembly.org/ns/rm");
